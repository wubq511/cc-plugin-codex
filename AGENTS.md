...
watchdog.mjs (supervised runner)
  ↕ task over stdin
Claude Code CLI (claude --output-format json)
```

- `scripts/cc-companion.mjs`: MCP entrypoint and six tool handlers. `cc_delegate` remains pending until the watchdog settles; the server remains responsive to cancellation.
...
- `scripts/lib/state.mjs`: schema-v4 per-job atomic files under the private workspace state directory. Writer leases use atomic cross-process acquisition plus heartbeat. Metadata is capped at 64 KiB; terminal retention is capped at 50 jobs, 30 days, and 100 MiB.
...
- Omitted `model` means Claude Code inherits its active Provider/model configuration. Explicit model identifiers are passed through unchanged; the plugin has no model catalogue.
