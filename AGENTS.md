...
- `scripts/lib/state.mjs`: schema-v7 per-job atomic files under the private workspace state directory. Writer leases use atomic cross-process acquisition plus heartbeat. Metadata is capped at 64 KiB; terminal bundles and standalone liveness evidence are pruned by age/count and the private evidence directory is capped at 100 MiB except for protected active/orphaned diagnostics.
