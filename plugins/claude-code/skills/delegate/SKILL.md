...
4. **Suggest review**: After a completed task, tell the user:
   > "Task completed. Run `/claude-code:review` to review the changes, or `/claude-code:review --adversarial` for a deeper review."
...
- For long-running tasks, use `background=true` and check progress with `/claude-code:status`.
