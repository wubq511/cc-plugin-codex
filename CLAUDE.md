...
- **Dynamic model routing** (`scripts/lib/routing.mjs`): Per-job active authority resolution with selector classification (inherited/alias/native), route snapshot construction (no secrets), child env construction (fixed strip of all inherited `ANTHROPIC_*` + Bedrock/Vertex flags, then allowlisted injection), and bounded display resolution. The default authority is **cc-profile-switch** (`${CC_PROFILE_SWITCH_HOME:-~/.cc-profile-switch}/config.json` → `lastUsedProfile` → `profiles/<name>/claude-home/settings.json` + common `api-settings.json`). When cc-profile-switch is absent, a Claude-settings adapter reads `<CLAUDE_CONFIG_DIR>/settings.json`, and an explicit `active-profile.json` fixture remains for tests/legacy. The authority is re-read fresh on every `cc_delegate` / `cc_resolve_route` / liveness probe — a profile switch takes effect on the next delegation without restarting Codex. Profile env is merged with precedence `common api-settings < profile settings.json`, then allowlisted: only `ANTHROPIC_[A-Z0-9_]+` and a fixed set of Claude routing flags may enter the child env. `PATH`, `NODE_OPTIONS`, `HOME`, `USER`, `SHELL`, `CLAUDE_CONFIG_DIR`, arbitrary env keys, and profile-declared `stripInherited` lists are rejected. Secrets (env values) exist only in the spawned child env — never in snapshots, fingerprints, state, diagnostics, or MCP output. The profile's `claude-home` becomes the child-only `CLAUDE_CONFIG_DIR`.
